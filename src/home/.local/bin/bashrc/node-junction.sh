#!/bin/bash
#
# Script: ~/.local/bin/bashrc/node-junction.sh
# Descrição: Funções para gerenciamento de junctions do Node.js no Windows
# ==========================================================================================

# Source das funções de display
if [ -f "${HOME}/.config/bashrc/00-bash-functions.sh" ]; then
    source "${HOME}/.config/bashrc/00-bash-functions.sh"
else
    displayAction() { echo ">>> $*"; }
...
if [ -f "${HOME}/.local/bin/bashrc/node-current.sh" ]; then
    source "${HOME}/.local/bin/bashrc/node-current.sh"
fi
