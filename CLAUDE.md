...
> **Sobre `~/bin` vs `~/.local/bin`:** <li> `~/bin/` é usado por este projeto como localização de alguns utilitários (ex.: `git-info`, `node-default`, ou `show-versions`, entre outros) porque o Git Bash já adiciona esse diretório ao PATH automaticamente — não precisamos gerenciar PATH para nossos scripts. <li> `~/.local/bin` contém binários instalados via Windows (ex.: `claude.exe` do instalador nativo do Claude Code, `jq.exe`, `wget.exe`, e outros); o RC Script `bash-junctions.sh` garante que esse caminho esteja no PATH.
